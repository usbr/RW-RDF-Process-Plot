...
verification_salt_process_plot <- function(scen_nm,process_scen,plot_scen) { 
  ...
  file_dir <- file.path(results_dir,scen_nm)
  ...
  if (file.exists(file.path(file_dir,paste0(scen_nm,".RDS"))) && process_scen == F) {
    message(paste('Reading from already processed RDS'))
    
    df_annual <- readRDS(file.path(file_dir,paste0(scen_nm,".RDS")))
    ...
    #### #### A. Read flow data  ####  ####
    ################################################################################
    message(paste('Processing',scen_nm))
    
    # rdf_slot_names(read.rdf(file.path(file_dir,"SaltVerification.rdf")))
    ...
    df_annual <- rdf_aggregate(rwa1, rdf_dir = file_dir) #MUST be in results folder
    ...
    df_annual$Scenario = scen_nm 
    
    saveRDS(df_annual,file.path(file_dir,paste0(scen_nm,".RDS")))
  ...
  if (plot_scen == T) {
   ...
   for (i in 1:length(nodes)) {
    ...
    if(printfigs==T){ ggsave(filename = file.path(fig_dir,paste(nodenums[i],nodes[i],"AnnPlot",scen_nm,".png")), width = gage_widths[1],height = gage_heights[1])}
    ...
    if(printfigs==T){ write.csv(x = df_csv,
                                file = file.path(data_dir,paste(nodenums[i],nodes[i],"Data",scen_nm,".csv")))}
    #### annual residuals ####
    if(T){
      # annual residual
      r1 <- diff %>%
      ...
      # if(printfigs==T){ ggsave(filename = file.path(file_dir,nodes[i],paste0("Grph Ann Resid",nodes[i]," ",scen_nm,".png")), width = gage_widths[2],height = gage_heights[2])}
      ...
      # if(printfigs==T){ ggsave(filename = file.path(ofigs,nodes[i],paste0("Grph Ann Cumsum Resid",nodes[i]," ",scen_nm,".png")), width = gage_widths[2],height = gage_heights[2])}
    ...
    if (dim(annstats)[1]==length(nodes)) { #this should mean you ran all of the traces
      ...
      write.csv(annstats[,2:dim(annstats)[2]],file = file.path(file_dir,paste0("AnnualVerificationStats",scen_nm,".csv")))
    } else {
      write.csv(annstats,file = file.path(file_dir,paste0("Partial_AnnualVerificationStats",scen_nm,".csv")))
    }
    ...
    if(T){
      ...
      if (dim(monstats)[1]==length(nodes)) { #this should mean you ran all of the traces
        ...
        write.csv(monstats[,2:dim(monstats)[2]],file = file.path(file_dir,paste0("MonthlyVerificationStats",scen_nm,".csv")))
      } else {
        write.csv(monstats,file = file.path(file_dir,paste0("Partial_MonthlyVerificationStats",scen_nm,".csv")))
      }
    } #### end monthly ####
