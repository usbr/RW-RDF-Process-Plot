...
zz_all <- scen_res2 %>%
  dplyr::filter(Year %in% yrs, Variable %in% c("Powell.Outflow",
                                               "Powell.Inflow","19_flow_bluf","16_flow_grut",
                                                "Mead.Outflow",
                                                "Havasu.Outflow",
                                               "AboveImperialDamColoradoR.Outflow",
                                                "8_flow_cisco")) %>%
...
hist <- read_xlsx(file.path(getwd(),'data/HistSLOAD_Flow.xlsx'))
...
## create a pdf  
pdf(file.path(oFigs,paste0("FlowAnnClouds_MinMaxLines=",MinMaxLines,"_",Figs,".pdf")), width= width, height= height)
