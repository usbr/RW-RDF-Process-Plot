...
rw_agg_file <- "SaltMassGains_rwagg_monthly.csv"
...
y_lab = "Salt Mass (thousand tons/mo)"
...
limit_date = as.Date("2030-12-01")
limit_date = as.Date("2026-12-01")
...
## create a pdf  
pdf(file.path(file_dir,paste0("Gain_Monthly_SaltMass_",Figs,".pdf")), width=9, height=6)
...
for (i in 1:length(vars)) {
  ...
  p <- df_ub %>%
    ...
    geom_line(aes(x=Date, y=MinOut, color=Scenario, group=Scenario),linetype = "dotted",size = 1) +  #bound with min max
    geom_line(aes(x=Date, y=MaxOut, color=Scenario, group=Scenario),linetype = "dotted",size = 1)  +
