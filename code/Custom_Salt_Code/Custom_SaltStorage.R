...
## create a pdf  
pdf(file.path(oFigs,paste0("SaltStorage_",Figs,".pdf")), width= width, height= height)
...
for (i in 1:length(unique(scen_res$Variable))) {
  ...
  title = paste("Median",variable) 
  # ylims <- c(400,600)
  
  
  p4 <- df %>%
    ggplot(aes(x = factor(Year), y = Med, color = Scenario, group = Scenario, linetype = Scenario, shape = Scenario)) + theme_light() +
