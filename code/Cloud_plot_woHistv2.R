...
gga <- gg + geom_ribbon(data = subset(zz,Scenario %in% scens),aes(ymin=Min, ymax=Max, fill = Scenario),  #CF: need to subset so don't create for mean 
                        alpha = 0.3, linetype = 2, size = 0.5*Medians) + 
  scale_fill_manual(name, 
                    values = plotColors, guide = guide_legend(order=1),
                    labels = str_wrap(scens[], 15)) + scale_color_manual(name,
                                                                             values = plotColors, guide = guide_legend(order=1),
                                                                             labels = str_wrap(scens, 15))  +
...
ggc <- gg + 
  ...
  geom_ribbon(data = subset(zz,Scenario %in% scens),aes(ymin=Min, ymax=Max, fill = Scenario),  #CF: need to subset so don't create for mean 
              alpha = 0.3, linetype = 2, size = 0.5*Medians) +
...
# add_logo_horiz(gg)
ggc <- ggc + labs(title = title)# +,subtitle = subtitle) 
