...
results_nm <- "June2021" #"Aug2021" #results dir folder 
...
#with CRMMS ICs 
mtom <- paste0("Trace", sprintf("%02d", 4:38))#paste0("Trace", 4:38)
...
# unique(zz$Scenario) %in% jun_st
# unique(zz$Scenario) %in% jun_st_nodo
zz <-  zz %>%
  mutate(ScenarioGroup = case_when(
    Scenario %in% jun_st ~ "June 2021 w DRO",
    Scenario %in% jun_st_nodo ~ "June 2021 no DRO", 
    TRUE ~ "BAD"))
...
dim(df) #10 timesteps in Aug, 339 timesteps/22400 
...
# test <- df %>%   filter(Variable == "CRSP Total") %>% 
#   filter(Year == 2022)
names(df)
dim(df)
...
df %>% group_by(Variable,Scenario,TraceNumber) %>%
  summarise(Value=sum(Value)) %>%
  filter(Variable == "CRSP Total") %>%
